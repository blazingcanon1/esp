...
class LoadInputsAndTargets(object):
    ...
    def __init__(
        ...
        self._loaders = {}
        if mode not in ["multiasr", "asr", "tts", "mt", "vc"]:
    ...
    def __call__(self, batch):
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    if self.mode=="multiasr":
                        visual=np.fromstring(base64.b64decode(inp["visual"]), dtype=np.float32)
                        x=np.hstack((x, np.tile(visual,(np.shape(x)[0],1))))
        ...
        if self.mode == "asr" or self.mode == "multiasr":
